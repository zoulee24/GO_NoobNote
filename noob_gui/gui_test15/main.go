...
type MyMainWindow struct {
	*walk.MainWindow
	...
	serialcanvas *walk.Canvas
	currentBounds walk.Rectangle
}

//不能改变
const FORMATWIDGET int = 90		//框框宽
const FORMATHEIGHT int = 55		//框框高

//可以改变
var MAXCOUNT float64 = 500	//X轴坐标最大值
var MAXSerial int = 6000		//Y轴坐标最大值
var MINSerial int = -6000		//Y轴坐标最小值

func main() {

	mainsw, sw := NewSerialWindow()
	go Serial(sw)
	if _, err := mainsw.Run(); err != nil {
		log.Fatal(err)
	}
}

func NewSerialWindow() (MainWindow, *MyMainWindow) {
	sw := new(MyMainWindow)

	SerialWindow := (MainWindow{
		...
		Children: []Widget{
			CustomWidget{
				AssignTo:            &sw.paintWidget,
				ClearsBackground:    true,
				InvalidatesOnResize: true,
				Paint:               sw.CoordinateSystemInit,
			},
		},
	})
	return SerialWindow, sw
}

func (mw *MyMainWindow) SerialWindowInit() error {
	...
	if err := mw.serialcanvas.DrawRectanglePixels(rectPen, bounds); err != nil {
		return err
	}
	...
	//放置0
	if err := mw.serialcanvas.DrawTextPixels(
		"0", 
		font, 
		walk.RGB(10, 10, 10), 
	...
	for key, _ := range lines_Width {
		if key % 2 == 0 {
			...
			if err := mw.serialcanvas.DrawLinePixels(linePen, lines_Width[key-1], lines_Width[key]); err != nil {
				return err
			}
			...
			if err := mw.serialcanvas.DrawTextPixels(nowcount_str, font, walk.RGB(10, 10, 10), bounds_font, walk.TextWordbreak); err != nil {
				return err
			}
			count++
		}
	...
	for key, _ := range lines_Height {
		if key % 2 == 0 {
			...
			if err := mw.serialcanvas.DrawLinePixels(linePen, lines_Height[key-1], lines_Height[key]); err != nil {
				return err
			}
			...
			if err := mw.serialcanvas.DrawTextPixels(nowserial_str, font, walk.RGB(10, 10, 10), bounds_font, walk.TextWordbreak); err != nil {
				return err
			}
		}
	}
